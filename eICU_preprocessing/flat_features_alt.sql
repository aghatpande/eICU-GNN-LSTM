...
-- creates a materialized view flatalt which looks like this:
/*
...
drop materialized view if exists flatalt cascade;
create materialized view flatalt as
  ...
  select distinct la.patientunitstayid, p.gender, p.age, p.ethnicity, p.admissionheight, p.admissionweight,
    ...
    inner join labelsalt as la on la.patientunitstayid = p.patientunitstayid;