-- MUST BE RUN AFTER labelsalt.sql

-- creates a materialized view diagnosesalt which looks like this:
/*
...
-- delete the materialized view diagnosesalt if it already exists
drop materialized view if exists diagnosesalt cascade;
create materialized view diagnosesalt as
  ...
  select d.patientunitstayid, d.diagnosisstring
    ...
    inner join labelsalt as l on l.patientunitstayid = d.patientunitstayid
  ...
  select ph.patientunitstayid, ph.pasthistorypath as diagnosisstring
    ...
    inner join labelsalt as l on l.patientunitstayid = ph.patientunitstayid
  ...
  select ad.patientunitstayid, ad.admitdxpath as diagnosisstring
    ...
    inner join labelsalt as l on l.patientunitstayid = ad.patientunitstayid
    inner join patient as p on p.patientunitstayid = ad.patientunitstayid
    where ad.admitdxenteredoffset < 1440;
-- It extracts data from 1486 patients from the eICUdemo database.
